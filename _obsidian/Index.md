---
type: MOC
title: Index — python-ai-learning
updated: 2026-04-12
---

# python-ai-learning — Map of Content

Vault de référence pour le projet d'apprentissage Python / AI de Benoit Brisson.

---

## Navigation rapide

| Note | Contenu |
|---|---|
| [[Environnements]] | Specs GX10 (Ubuntu) et Laptop Windows |
| [[Progression]] | État d'avancement des notebooks |
| [[Structure-Repo]] | Arborescence et intention de chaque module |
| [[Philosophie]] | Principes de travail et règles pédagogiques |
| [[Jupyter-Gotchas]] | Pièges Jupyter par environnement |
| [[Outils]] | Inventaire des outils installés |

---

## Statut rapide

- **Env actif** : `ai_learning` (Python 3.11, ipykernel) — présent sur GX10 et Windows
- **Repo GitHub** : `bbrisson/python-ai-learning`
- **Module en cours** : `01-environnement` → notebook `06-quarto.ipynb` (EN COURS)
- **Prochain notebook** : `05-vscode-cursor.ipynb` (à faire)

---

## Règle d'or du projet

> Auditer avant d'installer. Expliquer POURQUOI avant le COMMENT. Tout versionner.
